#include "jiantin.h"
struct JianTin *jt_link[JianTin_MAX_LEN];
int link_len = 0;
...
void jiantin_del(unsigned long long sql_id)
{
	...
	{
		if (jt_link[j]->sql_id == sql_id)
		{
			// 释放 jiantin 指向的内存
			free_JianTin(jt_link[j]);
			// 释放 jianting 的内存
			free(jt_link[j]);
			// 将最后一个监听移动过来，防止出现空缺//移除对 jiantin 的指向
			link_len = link_len - 1;
			jt_link[j] = jt_link[link_len];
			// 设为空，方式被其他地方误读已经释放的位置
			jt_link[link_len] = NULL;
			return;
		}
	}
}
...
void jiantin_print()
{
	...
	{
		get_data_str(jt_link[i]->data, jt_link[i]->data_len, data_list[jt_link[i]->name_id].ID, tmp, jt_link[i]->data_len);
		Serial.printf("jiantin_print  %d	%llu	%s %c %s\r\n", i, jt_link[i]->sql_id, jt_link[i]->name, jt_link[i]->fuhao, tmp);
	}
}
...
int jiantin_loop()
{
	...
	for (int i = 0; i < link_len; i++)
	{
		if (1 == is_true(data_list[jt_link[i]->name_id].ID, data_list[jt_link[i]->name_id].data, jt_link[i]->fuhao, jt_link[i]->data))
		{
			// 符合此监听项的条件，需要向服务发送警告
			// 检查是否已经有警告触发
			if (jt_link[i]->warn == NULL)
			{
				jt_link[i]->warn = (struct Udpwarn *)malloc(sizeof(struct Udpwarn));
				if (jt_link[i]->warn == NULL)
				{
					end = end - 1;
					continue;
				}
				str_len = sprintf(tmp, "%llu", jt_link[i]->sql_id);
				...
				jt_link[i]->warn->id = jt_link[i]->sql_id;
				jt_link[i]->warn->cmsg = MESSAGE;
				strcpy(tmp2, tmp);	   // 电脑上复制'\0'了，希望这里也复制了吧?
				*(tmp2 + str_len) = 0; // 末尾设置成0,手动 0
				jt_link[i]->warn->str_waring = tmp2;
				set_warn(jt_link[i]->warn);
			}
			else if (jt_link[i]->warn->status == NOT_WARN)
			{
				set_warn(jt_link[i]->warn);
			}
		}
		else
		{
			...
			if (jt_link[i]->warn->status != NOT_WARN)
			{
				warn_del_warn(jt_link[i]->warn);
				jt_link[i]->warn->status = NOT_WARN;
				jt_link[i]->warn->time = millis(); // 记录一个时间戳，超过多长时间都没有触发之后再回收内存
			}
			else if (millis() - jt_link[i]->warn->time > WARN_DELETE_OUTTIME_MS)
			{
				// 到达指定时间，回收内存
				...
				// 释放 warm 指向的内存
				free((void *)(jt_link[i]->warn->str_waring));
				// 因为不同的位置，这里的字符串不一定是申请来的内存，所以不封装
